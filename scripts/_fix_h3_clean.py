#!/usr/bin/env python3
"""Fix h3-01.json and produce _write-h3-01.py."""
import json
import shutil
import sys
...
def patch_blocks(data):
    ...
    for block in data["theory"]:
        ...
        if t == "list" and "Volyn" in str(block.get("items", "")):
            block["content"] = "Khronolohiia pryiednannia"
            block["items"] = b.page6_items()
        if t == "list" and "Yahailo" in block.get("content", ""):
            block["content"] = "Za uhodoiu Yahailo musiv"
            block["items"] = b.page7_yahailo()
        if t == "steps":
            block.update({"steps": b.vitovt_steps(), "content": "Polityka Vitovta (1392–1430 rr.)", "kicker": "Piat punktiv."})
        if t == "flip-cards":
            block.update({
                "flips": b.page27_flips(),
                "content": "Kripatstvo i panщina",
                "kicker": "Natysny kartku, shchob perehornuty.",
            })
        ...
        if t == "compare" and block.get("content") == "Seliany":
            block["columns"] = [
                {"title": "Pokhozhi", "items": ["osobysto vilni", "sluhy i voloky", "danynyky"]},
        ...
        if t == "story" and block.get("content", "").startswith("Магdeбurзьке") or (t == "story" and "Магdeбurзьке" in block.get("content", "")[:20]):
    ...
    for item in data["homework"]:
        if item["id"] in expl_map:
            item["explanation"] = expl_map[item["id"]]
    return data
...
def sync_writer(data):
    ...
    snap = json.dumps(data["theory"], ensure_ascii=False)
    notes = json.dumps(data["notes"], ensure_ascii=False)
    cards = json.dumps(data["quizCards"], ensure_ascii=False)
    inject = f'''

def _theory():
    return json.loads({snap!r})


def _notes():
    return json.loads({notes!r})


def _quiz_cards():
    return json.loads({cards!r})


'''
    text = text.replace("def build_lesson():", inject + "def build_lesson():")
    text = text.replace('"theory": "PLACEHOLDER"', '"theory": _theory()')
    text = text.replace('"notes": "PLACEHOLDER"', '"notes": _notes()')
    text = text.replace('"quizCards": "PLACEHOLDER"', '"quizCards": _quiz_cards()')
    # fix homework expl in writer source
    old_expl = '''    expl = {
        38: "Магdeбurзьке правo — samovriaduvannia mist.",
        39: "Генuезька форteця v Sudaku.",
        40: "Panщina — bezkoshtovna pratsia kripaka.",
        41: "Yurii Drohobych — «Prognostik».",
        42: "Mukachivskyi zamok — Zakarpattia.",
        43: "1385 r. — Krevska uniia.",
    }'''
    new_expl = '''    expl = {
        38: "Magdeburzke pravo — samovriaduvannia mist.",
        39: "Henuetska fortecia v Sudaku.",
        40: "Panщina — obov'iazkova pratsia na poli pana.",
        41: "Yurii Drohobych — «Prohnostyk» 1483 r.",
        42: "Mukachivskyi zamok — Zakarpattia.",
        43: "1385 r. — umovy Krevskoi uniï.",
    }'''
    text = text.replace(old_expl, new_expl)
    WRITE.write_text(text, encoding="utf-8")


def main():
    data = json.loads(OUT.read_text(encoding="utf-8"))
    data = patch_blocks(data)
    data = deep_replace(data, UK)
    # second pass with proper Ukrainian
    UA = {
        "Polshcha": "Польща",
        "Polityka Polshchi na ukrainskykh zemliakh": "Політика Польщі на ukrainskykh zemliakh",
        "Tri napriamy polityky": "Tri napriamy polityky",
